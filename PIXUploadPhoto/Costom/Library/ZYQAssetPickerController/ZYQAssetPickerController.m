...
//    checkedIcon     = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:[NSString stringWithFormat:@"ZYQAssetPicker.Bundle/Images/%@@2x.png",(!IS_IOS7) ? @"AssetsPickerChecked~iOS6" : @"AssetsPickerChecked"]]];
    checkedIcon = [UIImage imageNamed:@"btn-checked"];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row==ceil(self.assets.count*1.0/columns)) {
        ...
        if (_numberOfVideos == 0)
            title = [NSString stringWithFormat:NSLocalizedString(@"%ld 張照片", nil), (long)_numberOfPhotos];
        else if (_numberOfPhotos == 0)
            title = [NSString stringWithFormat:NSLocalizedString(@"%ld 部影片", nil), (long)_numberOfVideos];
        else
            title = [NSString stringWithFormat:NSLocalizedString(@"%ld 張照片, %ld 部影片", nil), (long)_numberOfPhotos, (long)_numberOfVideos];
...
- (void)setTitleWithSelectedIndexPaths:(NSArray *)indexPaths
{
    ...
    if (photosSelected && videoSelected)
        format = NSLocalizedString(@"已選擇 %ld 個項目", nil);
    
    else if (photosSelected)
        format = (indexPaths.count > 1) ? NSLocalizedString(@"已選擇 %ld 張照片", nil) : NSLocalizedString(@"已選擇 %ld 張照片 ", nil);
    
    else if (videoSelected)
        format = (indexPaths.count > 1) ? NSLocalizedString(@"已選擇 %ld 部影片", nil) : NSLocalizedString(@"已選擇 %ld 部影片 ", nil);
...
- (NSString *)accessibilityLabel
{
    ...
    return [label stringByAppendingFormat:NSLocalizedString(@"%ld 張照片", nil), (long)[self.assetsGroup numberOfAssets]];
}
...
@interface ZYQAssetGroupViewController()<UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
- (void)setupButtons
{
    ...
    if (picker.showCancelButton)
    {
        ...
        self.navigationItem.rightBarButtonItem =
        [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"相機", nil)
                                         style:UIBarButtonItemStylePlain
                                        target:self
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if (image) {
        ...
        SelectAlbumsTableView *selectAlbumsTableView = [[SelectAlbumsTableView alloc] init];
        [selectAlbumsTableView setFrame:CGRectMake(0 , 0 , self.view.frame.size.width - 40 , self.view.frame.size.height - self.navigationController.navigationBar.frame.size.height - 40)];
        [[selectAlbumsTableView layer] setCornerRadius:10.0];
        selectAlbumsTableView.selectImageArray = [array mutableCopy];
        selectAlbumsTableView.alertView = alertView;
        [alertView setButtonTitles:NULL];
        [alertView setContainerView:selectAlbumsTableView];
        [alertView show];
    }else {
...
- (void)showNotAllowed
{
    ...
    title.text              = NSLocalizedString(@"此應用無法使用您的照片或影片。", nil);
    title.font              = [UIFont boldSystemFontOfSize:17.0];
    title.textColor         = [UIColor colorWithRed:129.0/255.0 green:136.0/255.0 blue:148.0/255.0 alpha:1];
    title.textAlignment     = NSTextAlignmentCenter;
    title.numberOfLines     = 5;
    
    message.text            = NSLocalizedString(@"您可以在「隱私設置」中啟用存取。", nil);
...
- (void)showNoAssets
{
    ...
    title.text              = NSLocalizedString(@"沒有照片或影片。", nil);
    title.font              = [UIFont systemFontOfSize:26.0];
    title.textColor         = [UIColor colorWithRed:153.0/255.0 green:153.0/255.0 blue:153.0/255.0 alpha:1];
    title.textAlignment     = NSTextAlignmentCenter;
    title.numberOfLines     = 5;
    
    message.text            = NSLocalizedString(@"您可以使用 iTunes 將照片和影片\n同步到 iPhone。", nil);
